...
import { requestCarrierDocs } from "../services/api";
...
const DOC_TYPE_OPTIONS: { value: string; label: string; description: string }[] = [
  { value: "MC_AUTHORITY",      label: "MC Operating Authority", description: "FMCSA authority letter" },
  { value: "W9",                label: "W9 Tax Form",            description: "IRS W-9" },
  { value: "VOID_CHECK",        label: "Voided Check",           description: "For ACH payments" },
  { value: "CARRIER_AGREEMENT", label: "Carrier Agreement",      description: "Signed contract" },
  { value: "NOA",               label: "Notice of Assignment",   description: "NOA / factor letter" },
  { value: "COI",               label: "Certificate of Insurance", description: "Liability + cargo" },
  { value: "CDL",               label: "Driver's License",       description: "CDL copy" },
  { value: "OTHER",             label: "Other",                  description: "Other document" },
];

export default function CarrierDocumentRequestModal({ carrierId, carrierName, onClose, onRequestCreated }: Props) {
  const [selectedTypes, setSelectedTypes] = useState<Set<string>>(new Set(["W9", "COI"]));
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    setError(null);
    const res = await requestCarrierDocs({
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 300 }}
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Request Documents</h2>
        ...
        {result ? (
          <div>
            ...
            <label style={lblStyle}>Share this link with the carrier</label>
            <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
              <input readOnly value={result.magic_link} style={{ ...inpStyle, flex: 1, fontFamily: "monospace", fontSize: 12, color: "var(--mist)" }} />
            ...
            <p style={{ fontSize: 12, color: "#475569", marginBottom: 20 }}>
              Share via WhatsApp, text, or email. The carrier or owner doesn&apos;t need an account — they just open the link and upload.
            </p>
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <button type="button" onClick={() => { onRequestCreated(); onClose(); }} style={btnAmberStyle}>Done</button>
            </div>
          </div>
        ) : (
          /* ── Form state ── */
          <div>
              ...
              {DOC_TYPE_OPTIONS.map(({ value, label, description }) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <div>
                      <span style={{ fontSize: 14, fontWeight: 600, color: checked ? "#93c5fd" : "var(--white)" }}>{label}</span>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={lblStyle}>Note for carrier <span style={{ color: "#475569", fontWeight: 400 }}>(optional)</span></label>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder="e.g. Please include the front and back of the CDL" style={{ ...inpStyle, resize: "vertical" }} />
            </div>

            <div style={{ marginBottom: 18 }}>
              <label style={lblStyle}>Recipient email <span style={{ color: "#475569", fontWeight: 400 }}>(optional)</span></label>
              <input type="email" value={recipientEmail} onChange={(e) => setRecipientEmail(e.target.value)} placeholder="owner@carrier.com" style={inpStyle} />
            </div>
