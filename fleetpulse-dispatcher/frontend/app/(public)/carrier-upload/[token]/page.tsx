...
import { validateCarrierUploadToken, uploadCarrierFile } from "../../../../services/api";
...
const DOC_LABELS: Record<string, string> = {
  MC_AUTHORITY:      "MC Operating Authority",
  W9:                "W9 Tax Form",
...
export default function CarrierUploadPage({ params }: { params: { token: string } }) {
  const { token } = params;
  ...
  useEffect(() => {
    validateCarrierUploadToken(token).then((res) => {
      if (res.error || !res.data) {
        ...
        for (const dt of data.doc_types) {
          initial[dt] = { state: "idle", fileName: null, error: null, issueDate: "", expiresAt: "" };
        }
        setUploads(initial);
  ...
  const setDocField = (docType: string, field: "issueDate" | "expiresAt", value: string) => {
    setUploads((prev) => ({ ...prev, [docType]: { ...prev[docType], [field]: value } }));
  };
  ...
  const handleFileChange = async (docType: string, file: File | null) => {
    ...
    setUploads((prev) => ({
      ...prev,
      [docType]: { ...prev[docType], state: "uploading", fileName: file.name, error: null },
    }));

    const res = await uploadCarrierFile(
      token,
      file,
      docType,
      up?.issueDate || undefined,
      up?.expiresAt || undefined,
    );

    if (res.error) {
      setUploads((prev) => ({
        ...prev,
        [docType]: { ...prev[docType], state: "error", fileName: file.name, error: res.error },
      }));
    } else {
      setUploads((prev) => {
        const next = { ...prev, [docType]: { ...prev[docType], state: "done" as UploadState, fileName: file.name, error: null } };
        if (Object.values(next).every((u) => u.state === "done")) setAllDone(true);
        return next;
      });
    }
  };
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "32px 16px 64px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>Compliance Documents</h1>
        </div>
        <div style={{ background: "#1e293b", borderRadius: 10, padding: "14px 16px", display: "grid", gap: 4 }}>
          ...
          {info!.carrier_mc && <Row label="MC#" value={info!.carrier_mc} />}
        </div>
        {info!.notes && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {info!.doc_types.map((docType) => {
          ...
          return (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: isDone ? 4 : 12 }}>
                <div>
              ...
              {!isDone && (
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
                    <div>
                      <label style={lblStyle}>Issue Date <span style={{ color: "#475569" }}>(optional)</span></label>
                      <input
                        type="date"
                        value={up?.issueDate || ""}
                        onChange={(e) => setDocField(docType, "issueDate", e.target.value)}
                        style={dateInputStyle}
                        disabled={isUploading}
                      />
                    </div>
                    <div>
                      <label style={lblStyle}>Expiry Date <span style={{ color: "#475569" }}>(optional)</span></label>
                      <input
                        type="date"
                        value={up?.expiresAt || ""}
                        onChange={(e) => setDocField(docType, "expiresAt", e.target.value)}
                        style={dateInputStyle}
                        disabled={isUploading}
                      />
                    </div>
...
const lblStyle: React.CSSProperties = {
  fontSize: 11, color: "#94a3b8", display: "block", marginBottom: 3,
};
